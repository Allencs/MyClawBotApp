...
- **一条 `bash scripts/install.sh` 完成「装环境 + 装 PM2 + 扫码 + 守护启动」全流程**
- 健康探针每分钟检测微信掉线 / Bot Token 失效，连续 3 次失败自动 `pm2 restart`
...
脚本会按顺序自动做：
1. 检测/安装 `nvm` + Node 20（已有 Node ≥ 20 则跳过）
2. 全局装 `openclaw@latest`
3. 装并启用 `@tencent-weixin/openclaw-weixin@latest`（ClawBot 微信插件）
4. 渲染 `config/openclaw.template.json` → `~/.openclaw/openclaw.json`，注入 `OPENROUTER_API_KEY`，权限 600
5. 全局装 PM2
6. **暂停一次**，提示「请准备好微信，按回车开始扫码」→ 终端弹出二维码 → 用 v8.0.70+ 微信扫码授权
7. `pm2 start pm2.ecosystem.cjs && pm2 save` 拉起守护进程

跑完后会打印一条 `pm2 startup` 提示，按它输出的 sudo 命令再跑一次即可实现「服务器重启自动拉起」。

可选参数：

```bash
bash scripts/install.sh --skip-login   # 已经登录过、跳过扫码
bash scripts/install.sh --skip-pm2     # 不动 PM2，仅装环境/渲染配置
bash scripts/install.sh --reinstall    # 强制重新渲染 openclaw.json（已有的会自动备份）
```
...
正常 1–3 秒内会收到 LLM 回复。看实时日志：`pm2 logs myclawbot`。
